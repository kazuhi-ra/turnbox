...
export const edgeCasesSuite = (adapters: AdapterList, sharedAdapters: AdapterList) => {
  describe.each(adapters)("%s — edge cases", (_, createAdapter) => {
    ...
    it("faces 5 is capped to 4: starts at face 1 and navigates to face 4", async () => {
      adapter = createAdapter({ faces: 5, duration: 200 });
    ...
    it("faces 6 is capped to 4: can navigate to face 4", async () => {
      adapter = createAdapter({ faces: 6, duration: 200 });
      adapter.goTo(4);
      await adapter.advanceTime(300);
      expect(adapter.getCurrentFace()).toBe(4);
      expect(adapter.isFaceShown(4)).toBe(true);
    ...
    it("uneven geometry (axis:X, even≠height): starts at face 1", () => {
      adapter = createAdapter({ faces: 4, height: 50, even: 30, axis: "X" });
      expect(adapter.getCurrentFace()).toBe(1);
    ...
    it("uneven geometry (axis:Y, even≠width): navigation works correctly", async () => {
      adapter = createAdapter({ faces: 4, width: 200, even: 120, axis: "Y", duration: 200 });
      adapter.goTo(3);
      await adapter.advanceTime(300);
      expect(adapter.getCurrentFace()).toBe(3);
      expect(adapter.isFaceShown(3)).toBe(true);
